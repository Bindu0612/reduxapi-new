import React, { Component } from 'react';
import './App.css';
import { connect } from 'react-redux';
import { getUsers } from './action';

class NewAp extends Component {

  onBlur = (event) => {
    let events = event.target;

    if (events.name === "form") {
      event.preventDefault();

      if (events[0].value !== "") {
        this.props.getUsers(events[0].value);
        events[0].value = "";
      }
    }

    else {
      if (events.value !== "") {
        this.props.getUsers(events.value);
        events.value = "";
      }
    }
  }

  render() {
    const { users, loading, userData } = this.props.users;

    return (
      <>
        <form name="form" onSubmit={this.onBlur}>
          <section>
            <div className="container h-100">

              <div className="row">

                <h1 className="text-center"> <span>  PROFILE HUB </span>  </h1>

                <div className="col-md-6">
                  <input type="text" className="form-control" name="textbox" id="txt1"
                    style={{ "background": "rgb(224, 235, 235)" }}
                    onBlur={this.onBlur} placeholder=" Type Username + Enter" />

                  < div className="card profile-card-1" >
                    < img src={users != null ? users.avatar_url :
                      "data:img/jpeg;base64,"}
                      alt="profile-img" className="profile" />

                    <div className="card-content">
                      <h2> {userData ? users != null ? users.login : "sorry, user not found!" : ""}</h2>
                      <h2> {userData ? users != null ? users.location : "" : ""}</h2>

                      <h3>{loading && "Getting results for you… please wait."}</h3>
                      <div className="footer">
                        <div className="row">
                          <div className="col-md-4 col-sm-4 col-xs-4">
                            <div className="row">
                              <div className="col-md-12 col-xs-6">
                                {users != null && users.public_repos}
                              </div>
                              <div className="col-md-12 col-xs-6">
                                PublicRepos
                                  </div>
                            </div>
                          </div>

                          <div className="col-md-4 col-sm-4 col-xs-4">
                            <div className="row">
                              <div className="col-md-12 col-xs-6">
                                {users != null && users.followers}
                              </div>
                              <div className="col-md-12 col-xs-6">
                                Followers
                                    </div>
                            </div>
                          </div>

                          <div className="col-md-4  col-sm-4 col-xs-4">
                            <div className="row">
                              <div className="col-md-12 col-xs-6">
                                {users != null && users.following}
                              </div>

                              <div className="col-md-12 col-xs-6">
                                Following
                                    </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </section>
        </form>
      </>
    )

  }

}
const mapStateToProps = (state) => {
  return { users: state }
}


const mapDispachToProps = (dispatch) => {
  return {
    getUsers: (user) => dispatch(getUsers(user)),
  }
};


export default connect(mapStateToProps, mapDispachToProps)(NewAp);
